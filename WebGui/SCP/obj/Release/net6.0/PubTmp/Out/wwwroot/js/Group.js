$(function () {
    ...
    $('#maintain-save').on('click', function () {
        if (Object.keys(insertdata).length > 0 ||Object.keys(updatedata).length > 0 || Object.keys(deletedata).length > 0) {
            $.ajax({
                type: 'POST',
                url: '/Group/DataChange',
                data: JSON.stringify({
                    insertdata: insertdata,
                    updatedata: updatedata,
                    deletedata: deletedata.groupId,
                }),
    ...
    $(document).on('click', '.edit-btn', function () {
        ...
        if ($(this).hasClass("collapsed")) {
            ...
            if (originformdata !== formData) {
                ...
                //將更新資料存入陣列
                updatedata[formDataObject["GroupId"]] = formDataObject;
    ...
    $(document).on('click','.delete-btn', function () {
        ...
        if (form.is(':visible')) {
            ...
            if (!deletedata['groupId']) {
                deletedata['groupId'] = [];
            }
            
            deletedata['groupId'].push(groupId);
            row.hide();
        }
    ...
    $('#confirm-btn').on('click', function (e) {
        ...
        //將更新資料存入陣列
        insertdata[formDataObject["groupId"]] = formDataObject;
        form[0].reset();
...
function CreateNewRow(formData, fieldToColumnIndex) {
    var groupId = formData.find(item => item.name === 'GroupId').value;

    
    var newRow = $(
        '<tr>' +
            '<td class="align-middle"></td>' + // GroupId
            '<td class="align-middle"></td>' + // GroupCName
            '<td class="align-middle"></td>' + // GroupEName
            '<td class="align-middle"></td>' + // LogoutTime
            '<td class="align-middle">' +
                `<button data-bs-toggle="collapse" data-bs-target="#edit-${groupId}" aria-expanded="false" aria-controls="${groupId}" class="btn edit-btn p-0">` +
                    '<i class="fa-solid fa-square-pen fa-xl" style="color: #40B1FF;"></i>' +
                '</button> ' +  
                `<button class="btn delete-btn p-0" id="delete-${groupId}"><i class="fa-solid fa-square-minus fa-xl" style="color: #E25E3E;"></i></button>` +
            '</td>' +
        '</tr>' +
        '<tr>' +
            '<td colspan="5" class="border-0 p-0">' +
            `<div id="edit-${groupId}" class="collapse ps-3">` +
                    '<form>' +
                        '<div class=" d-flex align-items-center mt-3 mb-3">' +
                            '<label class="me-5">群組代碼:</label>' +
                            '<input class ="form-control-plaintext w-25" name="GroupId" readonly />' +
                        '</div>' +
                        '<div class=" d-flex align-items-center mb-3">' +
                            '<label class=" me-3">群組中文名稱:</label>' +
                            '<input class="form-control p-0 w-25" name="GroupCName" autocomplete="off"  />' +
                        '</div>' +
                        '<div class=" d-flex align-items-center mb-3">' +
                            '<label class=" me-3">群組英文名稱:</label>' +
                            '<input class="form-control p-0 w-25" name="GroupEName" autocomplete="off" />' +
                        '</div>' +
                        '<div class=" d-flex align-items-center mb-3">' +
                            '<label class=" me-5">登出時間:</label>' +
                            '<input class="form-control p-0" name="LogoutTime" autocomplete="off" style="width:50px;" />' +
                        '</div>' +
                    '</form>' +
                '</div>' +
            '</td>' +
