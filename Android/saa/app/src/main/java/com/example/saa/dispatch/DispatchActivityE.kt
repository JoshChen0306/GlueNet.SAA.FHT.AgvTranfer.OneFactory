...
class DispatchActivityE : AppCompatActivity() {
    ...
    private fun sendData(start: String, end: String,rackId:String,wordOrder:String) {
        CoroutineScope(Dispatchers.Main).launch {
            ...
            //}
            var success = dbHelper.change_oPort(start, rackId,"1", "")
            if (success) {
        ...
        try {
            CoroutineScope(Dispatchers.Main).launch {
                ...
                val filteredPorts = oportList.filter {
                    (it.StationNo.startsWith("A") || it.StationNo.startsWith("B") || it.StationNo.startsWith("D")) && it.UseFlag == "Y" && it.BgnToEnd.isNullOrEmpty() && it.HaveFlag == "0"
                }
                ...
                if (filteredPorts.isNullOrEmpty()) {
                    ...
                    val adapter = ArrayAdapter(this@DispatchActivityE, android.R.layout.simple_spinner_item, filteredPorts.map { it.StationNo })
