...
class DispatchActivityA : AppCompatActivity() {
    ...
    private val buttonIds = listOf(
        R.id.buttonStationA6 to "A6",
        R.id.buttonStationA5 to "A5",
        R.id.buttonStationA4 to "A4",
        R.id.buttonStationA3 to "A3",
        R.id.buttonStationA2 to "A2",
        R.id.buttonStationA1 to "A1"
    )
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_dispatch_a)
    ...
    private fun sendData(start: String, end: String,rackId:String,wordOrder:String) {
        CoroutineScope(Dispatchers.Main).launch {
            ...
            if (success) {
                Toast.makeText(this@DispatchActivityA, "Date(oNeed) send success", Toast.LENGTH_SHORT).show()
            } else {
    ...
    private fun loadSpinnerData() {
        CoroutineScope(Dispatchers.Main).launch {
            ...
            val filteredPorts = allPorts.filter {
                it.stationNo.startsWith("B")&&it.useFlag == "Y" && it.bgnToEnd == null && it.haveFlag == "0"
            }

            if (filteredPorts.isEmpty()) {
                Toast.makeText(this@DispatchActivityA, "Failed to retrieve valid station data from the database.", Toast.LENGTH_SHORT).show()
            } else {
                val adapter = ArrayAdapter(this@DispatchActivityA, android.R.layout.simple_spinner_item, filteredPorts)
                adapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item)
                spnPort.adapter = adapter
                Toast.makeText(this@DispatchActivityA, "Successfully loaded ${filteredPorts.size} station records.", Toast.LENGTH_SHORT).show()
            }
        }
    }
    ...
    private fun setupStationButtons() {
        CoroutineScope(Dispatchers.Main).launch {
            ...
            for ((buttonId, stationNo) in buttonIds) {
                ...
                if (stationInfo != null && stationInfo.useFlag == "Y" && stationInfo.haveFlag == "3" && stationInfo.bgnToEnd == null) {
                    // 启用按钮并设置启用状态图像
