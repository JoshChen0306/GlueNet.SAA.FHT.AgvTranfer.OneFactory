...
## TaskType 設計（方案 A：配置檔多 TaskType）

### 問題描述

發送海康 RCS 任務時，TaskType 存在差異：
- **站內運輸（同樓層）** = `F001`
- **跨樓層運輸（需搭電梯）** = `F34`

目前系統使用固定的 `AGVTaskType`，無法區分這兩種情況。

---

### 設計方案

在配置檔新增跨樓層 TaskType，由 `ElevatorPathCalculator` 判斷是否跨樓層，自動選擇對應 TaskType。
...
新增 `CrossFloorTaskType` 屬性：

```csharp
[ConfigurationProperty("CrossFloorTaskType", DefaultValue = "F34")]
public string CrossFloorTaskType
{
    get { return (string)this["CrossFloorTaskType"]; }
    set { this["CrossFloorTaskType"] = value; }
}
```
...
觀察 `AGVDispatch` Log 中的 `TaskType:` 記錄，確認：
- 同樓層任務使用 `F001`
- 跨樓層任務使用 `F34`
...
1. [ ] 修改 `ConfigAGVModel.cs` 新增 `CrossFloorTaskType` 屬性
2. [ ] 修改 `ElevatorPathCalculator.cs` 新增 `IsCrossFloor` 方法
3. [ ] 修改 `Dispatch.cs` 的 `GetSchedulingTask` 方法
4. [ ] 修改 `HikAGV.config` 新增 `CrossFloorTaskType` 配置
5. [ ] 編譯測試
