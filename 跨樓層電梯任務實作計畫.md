...
客戶變更 TaskType 設計方式，從原本的「站內/跨樓層」二分法，改為**每條路線專屬 TaskType**。
...
> [!NOTE]
> **已確認**：1F↔2F、2F↔3F 路線不存在，上述對照表已涵蓋所有實際路線。

---

### 配置修改

#### [MODIFY] ConfigAGVModel.cs

位置：`ACC/HikAGVWebAPI/HikAGV/Models/ConfigAGVModel.cs`

新增 `CrossFloorTaskTypeMap` 屬性（路線對照表）：

```csharp
/// <summary>
/// 跨樓層 TaskType 對照表 (格式: "1F>3F:F13Test,3F>1F:F31Test,...")
/// </summary>
[ConfigurationProperty("CrossFloorTaskTypeMap", 
    DefaultValue = "1F>3F:F13Test,3F>1F:F31Test,3F>4F:F34Test,4F>3F:F43Test,2F>4F:F24Test,4F>2F:F42Test")]
public string CrossFloorTaskTypeMap
{
    get { return (string)this["CrossFloorTaskTypeMap"]; }
    set { this["CrossFloorTaskTypeMap"] = value; }
}
```

#### [MODIFY] HikAGV.config

位置：`ACC/HikAGVWebAPI/HikAGV/Config/HikAGV.config`

```xml
<AGVSettings 
    AGVMapCode="AA,BB,DD,FF" 
    AGVTaskType="F001" 
    CrossFloorTaskTypeMap="1F>3F:F13Test,3F>1F:F31Test,3F>4F:F34Test,4F>3F:F43Test,2F>4F:F24Test,4F>2F:F42Test"
    WarnContent="安全告警-前碰撞条触发,安全告警-后碰撞条触发" />
```

> [!TIP]
> **設定檔彈性設計**：客戶後續變更 TaskType 只需修改 `HikAGV.config`，無需重新編譯程式。

---

### 程式碼修改

#### [MODIFY] ElevatorPathCalculator.cs
                          ...
                          string taskTypeMap, string defaultTaskType)
{
    ...
    // 找不到對應路線，記錄警告並返回預設值
    mLog?.TraceOut($"Warning: No TaskType found for route {routeKey}, using default", Log.LogType.NONE);
    return defaultTaskType;
}
...
#### [MODIFY] Dispatch.cs - GetSchedulingTask
...
| 測試案例 | 起點 | 終點 | 起終樓層 | 預期 TaskType |
|---------|------|------|---------|--------------|
| 同樓層 2F | M1 | O1 | 2F→2F | **F001** |
| 同樓層 1F | A1 | C1 | 1F→1F | **F001** |
| 1F→3F | G1 | J1 | 1F→3F | **F13Test** |
| 3F→1F | J1 | G1 | 3F→1F | **F31Test** |
| 3F→4F | I1 | K1 | 3F→4F | **F34Test** |
| 4F→3F | K1 | I1 | 4F→3F | **F43Test** |
| 2F→4F | H1 | K1 | 2F→4F | **F24Test** |
| 4F→2F | K1 | H1 | 4F→2F | **F42Test** |

### 驗證方式

觀察 `AGVDispatch` Log 中的記錄：
```
TaskType: F13Test, Route: 1F>3F
```
...
1. [x] 修改 `ConfigAGVModel.cs` 新增 `CrossFloorTaskTypeMap` 屬性
2. [x] 修改 `ElevatorPathCalculator.cs` 新增 `GetTaskType()` 與 `ParseTaskTypeMap()` 方法
3. [x] 修改 `Dispatch.cs` 的 `GetSchedulingTask` 方法
4. [x] 修改 `HikAGV.config` 新增 `CrossFloorTaskTypeMap` 配置
5. [ ] 編譯測試
