...
class DispatchActivityA : AppCompatActivity() {
    ...
    private fun checkArea(scandata: String): Boolean {
        ...
        try {
            ...
            val area = (63 + user?.groupId!!.toInt()).toChar().toString()
            if (areasub == area)
                result = true
        }
        catch (e: Exception) {
    ...
    private fun sendData(start: String, end: String,rackId:String,wordOrder:String) {
        CoroutineScope(Dispatchers.Main).launch {
            var success = dbHelper.send_oNeed(start, end, rackId, wordOrder, null)
            if (success) {
        ...
        try {
            CoroutineScope(Dispatchers.Main).launch {
                ...
                if (filteredPorts.isNullOrEmpty()) {
                    ...
                    val adapter = ArrayAdapter(this@DispatchActivityA, android.R.layout.simple_spinner_item, filteredPorts.map { it.StationNo })
