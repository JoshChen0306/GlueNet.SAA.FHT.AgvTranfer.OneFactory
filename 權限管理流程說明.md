...
| 資料表 | 說明 |
|--------|------|
| `pRoute` | 路線定義（7 條客戶需求路線） |
| `pUserRoute` | 使用者路線權限對應（多對多） |

### 7 條路線定義

| # | RouteId | 路線名稱 | 起點區域 | 終點區域 |
|---|---------|----------|----------|----------|
...
| 檔案 | 路徑 | 說明 |
|------|------|------|
| RoutePermission_CreateTables.sql | 專案根目錄 | 資料表建立 SQL 腳本 |
| pRoute.cs | `WebGui/SCP/Models/` | 路線定義 Model |
| pUserRoute.cs | `WebGui/SCP/Models/` | 使用者路線權限 Model |
| RouteController.cs | `WebGui/SCP/Controllers/` | 路線權限管理 API |
| Index.cshtml | `WebGui/SCP/Views/Route/` | 路線權限管理 UI |

### 使用方式

1. 執行 `RoutePermission_CreateTables.sql` 建立資料表和預設資料
2. 以管理者身份登入系統
3. 進入「權限管理 > 路線權限管理」頁面
4. 選擇使用者，勾選欲授權的路線，儲存

> **注意**：1F 站內運輸（A, B, C, D, E, F）保持現狀，不納入路線權限管控。
